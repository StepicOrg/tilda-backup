...
function t860_init(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var wrapper=rec.querySelector('.t860');if(!wrapper)return;t_onFuncLoad('t_cards__moveClickOnCard',function(){t_cards__moveClickOnCard(recId)});t860_unifyHeights(recId);t860_translateBlock(recId);window.addEventListener('resize',t_throttle(function(){if(window.noAdaptive&&window.noAdaptive===!0&&window.isMobile)return;t860_unifyHeights(recId);t860_translateBlock(recId)}));window.addEventListener('load',function(){t860_unifyHeights(recId);t860_translateBlock(recId)});if(typeof jQuery!=='undefined'){$(wrapper).on('displayChanged',function(){t860_unifyHeights(recId);t860_translateBlock(recId)})}else{wrapper.addEventListener('displayChanged',function(){t860_unifyHeights(recId);t860_translateBlock(recId)})}
