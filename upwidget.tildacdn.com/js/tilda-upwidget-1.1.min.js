...
function deltaE(d,p){var n=RGB_to_LAB(d),h=RGB_to_LAB(p);n=Math.sqrt((n[0]-h[0])*(n[0]-h[0])+(n[1]-h[1])*(n[1]-h[1])+(n[2]-h[2])*(n[2]-h[2]));console.log("DeltaE: "+n);return n}function stringToRGBArray(d){if("undefined"==typeof d||0!=d.indexOf("rgb"))return[0,0,0];d=d.split("(")[1].split(")")[0].split(",");return 3>d.length?[0,0,0]:d}
function hexToRgb(d){d=d.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i,function(d,h,p,u){return h+h+p+p+u+u});var p=(d=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(d))?{r:parseInt(d[1],16),g:parseInt(d[2],16),b:parseInt(d[3],16)}:null;return d?[p.r,p.g,p.b]:null}function rgbToHex(d,p,n){return"#"+(16777216+(d<<16)+(p<<8)+n).toString(16).slice(1)};