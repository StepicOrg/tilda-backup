...
window.tildaForm.arProductsForStat.push(arProduct)}}
$jform.append(html)}
window.tildaForm.payment=function($jform,arNext){var html='';if($jform.data('formpaymentoff')=='y'){window.tcart={amount:0,currency:'',system:'',products:[]};window.tcart.system='none';return}
...
$jform.addClass('js-send-form-success');if(arNext.type=='link'){window.tcart={amount:0,currency:'',system:'',products:[]};if(arNext.message&&arNext.message>''&&$jform.find('.js-successbox').length>0){$jform.find('.js-successbox').html(arNext.message);$jform.find('.js-successbox').show()}
window.location.href=arNext.value;return!0}else{if(arNext.type=='form'){window.tcart={amount:0,currency:'',system:'',products:[]};$('#js-tilda-payment-formid').remove();var key='',val='';html='<form id="js-tilda-payment-formid" action="'+arNext.value.action+'" method="post"  style="position:absolue;opacity:0;width: 1px; height: 1px; left: -5000px;">';arNext.value.action='';for(key in arNext.value){val=arNext.value[key];if(typeof(val)!='function'&&val>''){html+="<input type='hidden' name='"+key+"' value='"+val+"' >"}}
html+='</form>';$('body').append(html);$('#js-tilda-payment-formid').submit()}else{if(arNext.type=='function'){var arArgs=arNext.value.args;eval(arNext.value.name+'($jform, arArgs);');return!1}else{window.tcart={amount:0,currency:'',system:'',products:[]};if(arNext.type=='text'&&arNext.message&&arNext.message>''&&$jform.find('.js-successbox').length>0){$jform.find('.js-successbox').html(arNext.message);$jform.find('.js-successbox').show()}}}}};window.tildaForm.stripeLoad=function(){if(window.stripeapiiscalled!==!0){var script=document.createElement("script");script.type="text/javascript";script.src="https://checkout.stripe.com/checkout.js";document.body.appendChild(script);window.stripeapiiscalled=!0}};window.tildaForm.stripePay=function($jform,arArgs){if(window.stripeapiiscalled!==!0){window.tildaForm.stripeLoad();window.setTimeout(function(){window.tildaForm.stripePay($jform,arArgs)},200);return!1}
...
$jform.data('successmessage','');if(arArgs.failureurl>''){window.location.href=arArgs.failureurl}else{$jpopup.find('.t706__cartwin-products').show();$jpopup.find('.t706__cartwin-prodamount-wrap').show();$jpopup.find('.t706__form-bottom-text').show();$jform.find('.t-form__inputsbox').show()}});return!1};window.tildaForm.sendStatAndShowMessage=function($jform,arArgs,sendStat){if(sendStat){var virtPage='/tilda/'+$jform.attr('id')+'/payment/';var virtTitle='Pay order in form '+$jform.attr('id');var virtPrice=arArgs.amount;var virtProduct=arArgs.description;if(window.Tilda&&typeof Tilda.sendEventToStatistics=='function'){var currency=$('#allrecords').data('tilda-currency');if(!currency){$('#allrecords').data('tilda-currency',arArgs.currency)}
Tilda.sendEventToStatistics(virtPage,virtTitle,virtProduct,virtPrice)}}
