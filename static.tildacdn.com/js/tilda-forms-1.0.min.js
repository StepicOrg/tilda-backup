...
$jform.find('input[type=text]:visible').val('');$jform.find('textarea:visible').html('');$jform.find('textarea:visible').val('');$jform.data('tildaformresult',{tranid:"0",orderid:"0"});$jform.trigger('tildaform:aftersuccess')};window.tildaForm.banktransferPay=function($jform,arArgs){if(arArgs&&arArgs.condition=='fast'){window.tildaForm.sendStatAndShowMessage($jform,arArgs,!0)}else{if(arArgs&&arArgs.html>''){$('#allrecords').append(arArgs.html);$('.t-banktransfer .t-popup__close').off('click');$('.t-banktransfer .t-popup__close').click(function(){$('body').removeClass('t-body_popupshowed');$('.t-banktransfer').remove();return!1});$('body').addClass('t-body_popupshowed');var $jbankform=$('#formbanktransfer'),arErrors;if($jbankform.length>0){$jbankform.off('submit');$jbankform.find('.t-submit').off('click');$jbankform.find('.t-submit').off('dblclick');$jbankform.submit(function(e){e.preventDefault();arErrors=window.tildaForm.validate($jbankform);if(arErrors&&arErrors.length>0){window.tildaForm.showErrors($jbankform,arErrors);return!1}
